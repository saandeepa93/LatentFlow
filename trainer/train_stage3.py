...
class LinearClassifier(nn.Module):
  def __init__(self, cfg):
    super().__init__()
    self.linear = nn.Linear(cfg.FLOW.IN_FEAT, cfg.FLOW.IN_FEAT )
    self.linear.weight.data.copy_(torch.eye(cfg.FLOW.IN_FEAT))
    self.linear.bias.data.zero_()

    self.fc = nn.Linear(cfg.FLOW.IN_FEAT, cfg.DATASET.N_CLASS)
    ...
    self.mean = nn.Parameter(torch.stack(mu))
    self.log_sd = nn.Parameter(torch.stack(log_sd))
  
  def log_prob(self, z):
    return -0.5 * log(2 * pi) - self.log_sd - 0.5 * (z - self.mean) ** 2 / torch.exp(2 * self.log_sd)

  def calc_likelihood(self, z):
    z = rearrange(z, 'b d -> b 1 d')
    log_p_batch = self.log_prob(z)
    log_p_all = log_p_batch.sum(dim=(2))
    return log_p_all/ (log(2) * self.n_pixel)

  def forward(self, x):
    z = self.linear(x)
    log_p = self.calc_likelihood(z)
    return F.softmax(log_p, -1)
...
def train(loader, epoch, model, classifier, optimizer, criterion, cfg, device, sample_weight, mu, sigma):
  robust = False
  avg_loss = []
  y_pred = []
  y_true = []
  ...
  model.eval()
  classifier.train()
  for b, (image, exp, fnames, indexes) in enumerate(loader,0):
    if cfg.DATASET.AUG2:
    ...
    # WARMUP LEARsNING RATE
    warmup_learning_rate(cfg, epoch, b, len(loader), optimizer)

    # FORWARD 
    with torch.no_grad():
      z, means, log_sds, *_ = model(image)
    log_probs = classifier(z)
    # log_probs = F.softmax(calc_likelihood(cfg, out, mu, log_sd, device, n_pixel), dim=-1)
    # loss = criterion(log_probs, exp, sample_weight)
    loss = criterion(log_probs, exp)
...
def validate(loader, model, classifier, criterion, device, sample_weight, mu, sigma):
  total_loss = []
  y_pred = []
  y_true = []
  n_pixel = cfg.FLOW.IN_FEAT
...
if __name__ == "__main__":
  ...
  for i in pbar:
    ...
    # TRAINING
    train_loss, y_pred_train, y_true_train = train(train_loader, i, model, classifier, optimizer, criterion, cfg, device, loss_weights, mu, log_sd)
    with torch.no_grad():
      val_loss, y_pred_val, y_true_val = validate(val_loader, model, classifier, criterion, device, loss_weights, mu, log_sd)
